...
class Account {
  ...
  void savetransactions(ofstream& outFile, int userID) {
        for (auto& it : borrowedDate) {
            outFile << userID << "," << it.first << "," << Timetostring(it.second) << ",BORROW\n";
        }
        for (auto& it : returnDate) {
            outFile << userID << "," << it.first << "," << Timetostring(it.second) << ",RETURN\n";
        }
    }
...
int main() {
    ...
    int choice;
    while (true) {
        cout << "\n" <<" Welcome to the Library Management System implemented by sohan and sateesh  "<< "\n";
